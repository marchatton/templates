...
Run all commands from the installed dev-browser skill directory (folder containing this `SKILL.md`).
In Codex this is typically: `$CODEX_HOME/skills/dev-browser/`.
...
```bash
cd "$CODEX_HOME/skills/dev-browser" && ./server.sh &
```
...
```bash
cd "$CODEX_HOME/skills/dev-browser" && npm i && npm run start-extension &
```
...
> **Run all scripts from the dev-browser skill directory** (typically `$CODEX_HOME/skills/dev-browser/`). The `@/` import alias requires this directory's config.

Execute scripts inline using heredocs:

```bash
cd "$CODEX_HOME/skills/dev-browser" && npx tsx <<'EOF'
