...
**Note:** If running `/wf-plan` with ultrathink enabled, automatically run `/deepen-plan` after plan creation for maximum depth and grounding.

Loop back to options after Simplify or Other changes until user selects `/wf-work` or `/plan_review`.
...
5. **After creation:**
   ...
   - Ask if they want to proceed to `/wf-work` or `/plan_review`
